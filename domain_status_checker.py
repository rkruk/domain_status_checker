...
def scan_domain(domain, dry_run=False, retries=2):
    domain = domain.strip()
...
def main():
    ...
    def scan_and_save(domain):
        if domain in already_scanned:
            return None
        result = scan_domain(domain, dry_run=args.dry_run)
    ...
    try:
        if args.threads > 1:
            with ThreadPoolExecutor(max_workers=args.threads) as executor:
                future_to_domain = {executor.submit(scan_and_save, domain): domain for domain in domains if domain not in already_scanned}
    ...
    summarize_results(results, summary_file=summary_path)
